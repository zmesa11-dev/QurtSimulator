...
export function SimulatorControls(props: SimulatorControlsProps) {
  const { t } = useLanguage();
  const [showAdvanced, setShowAdvanced] = useState(false);
  const [isMobileExpanded, setIsMobileExpanded] = useState(false);
    ...
    <div className="max-w-[1600px] mx-auto px-4 lg:px-8 py-4 md:py-6">
      ...
      <div className="md:hidden flex items-center justify-between bg-muted/40 p-3 rounded-lg border border-border/60 mb-2">
          <div className="flex flex-col">
              <span className="text-[10px] uppercase text-muted-foreground font-bold tracking-wider">Configuration</span>
              <div className="flex items-center gap-2 mt-0.5">
                  <span className="font-bold text-primary text-sm">{currency.format(props.price)}</span>
                  <span className="text-xs text-muted-foreground">•</span>
      ...
      <div className={`${!isMobileExpanded ? 'hidden' : 'grid'} md:grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 items-end mb-2 animate-in slide-in-from-top-2`}>
          <div className="md:col-span-4 space-y-4">
              <div className="flex justify-between items-center"><Label className="text-xs font-bold uppercase text-muted-foreground tracking-wide">{t('price')}</Label><span className="text-lg font-bold text-primary font-mono">{currency.format(props.price)}</span></div>
              <Slider min={100000} max={1500000} step={5000} value={[props.price]} onValueChange={(v) => props.setPrice(v[0])} className="py-2" />
          </div>
          <div className="md:col-span-4 space-y-4">
              <div className="flex justify-between items-center"><Label className={`text-xs font-bold uppercase tracking-wide flex items-center gap-1 ${!simulation.isCompliant ? "text-destructive" : "text-muted-foreground"}`}>{simulation.isCompliant ? t('downPayment') : <><AlertTriangle className="h-3 w-3"/> {t('warning_downpayment')}</>}</Label><span className={`text-lg font-bold font-mono ${!simulation.isCompliant ? "text-destructive" : "text-green-600"}`}>{currency.format(props.downPayment)}</span></div>
              <Slider min={0} max={props.price} step={1000} value={[props.downPayment]} onValueChange={(v) => props.setDownPayment(v[0])} className="py-2" />
          </div>
          <div className="md:col-span-2 space-y-4">
              <div className="flex justify-between items-center"><Label className="text-xs font-bold uppercase text-muted-foreground tracking-wide">{t('duration')}</Label><span className="font-bold text-sm">{props.durationYears} {t('years')}</span></div>
              <Slider min={5} max={30} step={1} value={[props.durationYears]} onValueChange={(v) => props.setDurationYears(v[0])} className="py-2" />
          </div>
          <div className="md:col-span-2 flex justify-end pb-1">
              <Button variant="ghost" size="sm" onClick={() => setShowAdvanced(!showAdvanced)} className="gap-2 text-xs font-semibold text-muted-foreground hover:text-foreground w-full md:w-auto">
                  <Settings2 className="h-4 w-4" />
                  {showAdvanced ? t('masquer') : t('configure')} 
                  {showAdvanced ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
              </Button>
          </div>
      </div>
      ...
      {showAdvanced && (isMobileExpanded || window.innerWidth >= 768) && (
          <div className="mt-6 md:mt-8 pt-6 border-t border-border/40 grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10 animate-in slide-in-from-top-2 fade-in duration-200">
              <div className="space-y-4">
                  <h3 className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider flex items-center gap-2"><Zap className="h-3 w-3"/> {t('charges_title')}</h3>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                      <div className="space-y-1"><Label className="text-[10px] text-muted-foreground">{t('municipal')}</Label><Input type="number" value={props.municipalTax} onChange={(e) => props.setMunicipalTax(Number(e.target.value))} className="h-7 text-xs bg-transparent border-0 border-b rounded-none px-0"/></div>
                      <div className="space-y-1"><Label className="text-[10px] text-muted-foreground">{t('school')}</Label><Input type="number" value={props.schoolTax} onChange={(e) => props.setSchoolTax(Number(e.target.value))} className="h-7 text-xs bg-transparent border-0 border-b rounded-none px-0"/></div>
                      <div className="space-y-1"><Label className="text-[10px] text-muted-foreground">{t('insurance')}</Label><Input type="number" value={props.insurance} onChange={(e) => props.setInsurance(Number(e.target.value))} className="h-7 text-xs bg-transparent border-0 border-b rounded-none px-0"/></div>
                      <div className="space-y-1"><Label className="text-[10px] text-muted-foreground">{t('hydro')}</Label><Input type="number" value={props.hydro} onChange={(e) => props.setHydro(Number(e.target.value))} className="h-7 text-xs bg-transparent border-0 border-b rounded-none px-0"/></div>
                      <div className="col-span-2 space-y-1"><Label className="text-[10px] text-muted-foreground">{t('maintenance')}</Label><Input type="number" value={props.maintenance} onChange={(e) => props.setMaintenance(Number(e.target.value))} className="h-7 text-xs bg-transparent border-0 border-b rounded-none px-0"/></div>
                  </div>
              </div>
              <div className="space-y-5">
                  <h3 className="text-[10px] font-bold uppercase text-primary tracking-wider flex items-center gap-2"><TrendingUp className="h-3 w-3"/> {t('strategy_title')}</h3>
                  <div className="space-y-4">
                      <div className="space-y-2"><div className="flex justify-between text-xs"><Label>{t('extra_monthly')}</Label><span className="font-bold text-primary">+{currency.format(props.extraMonthly)}</span></div><Slider min={0} max={2000} step={50} value={[props.extraMonthly]} onValueChange={(v) => props.setExtraMonthly(v[0])} /></div>
                      <div className="space-y-2"><div className="flex justify-between text-xs"><Label>{t('extra_annual')}</Label><span className="font-bold text-primary">+{currency.format(props.extraAnnual)}</span></div><Slider min={0} max={20000} step={500} value={[props.extraAnnual]} onValueChange={(v) => props.setExtraAnnual(v[0])} /></div>
                      <div className="space-y-2 pt-2"><div className="flex justify-between text-xs"><Label>{t('appreciation')}</Label><span className="font-bold text-green-600">+{props.appreciation}%</span></div><Slider min={0} max={8} step={0.5} value={[props.appreciation]} onValueChange={(v) => props.setAppreciation(v[0])} /></div>
                  </div>
              </div>
              <div className="space-y-4">
                  <h3 className="text-[10px] font-bold uppercase text-green-600 tracking-wider flex items-center gap-2"><Wallet className="h-3 w-3"/> {t('costs_title')}</h3>
                  <div className="space-y-2 text-xs">
                      <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">{t('downPayment')}</span><span className="font-medium">{currency.format(props.downPayment)}</span></div>
                      <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">{t('welcome_tax')}</span><span>{currency.format(props.welcomeTax)}</span></div>
                      <div className="flex justify-between py-1 border-b border-border/30"><span className="text-muted-foreground">{t('notary')}</span><span>{currency.format(props.notaryFees + props.startupFees)}</span></div>
                      <div className="flex justify-between pt-3"><span className="font-bold uppercase text-[10px] tracking-wider text-muted-foreground">{t('cash_required')}</span><span className="font-bold text-base text-foreground">{currency.format(props.totalCashRequired)}</span></div>
                  </div>
              </div>
          </div>
      )}
