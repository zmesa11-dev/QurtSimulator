...
export default function Home() {
  ...
  // Fonction utilitaire traduite
  const formatMonthsToYears = (m: number) => {
      const y = Math.floor(m / 12);
      const remM = m % 12;
      // Utilisation de t('years') et t('col_month')
      const labelYear = t('years'); 
      const labelMonth = t('col_month'); // Assurez-vous d'avoir 'Mois'/'Month' dans votre dico, sinon mettez 'mois'/'mo'
      
      // Petit hack si 'col_month' n'existe pas dans votre dico actuel : on utilise 'mois' par défaut
      const moLabel = labelMonth || "mois";

      if (y === 0) return `${remM} ${moLabel}`;
      if (remM === 0) return `${y} ${labelYear}`;
      return `${y} ${labelYear} ${remM} ${moLabel}`;
  }
          ...
          <div className="max-w-[1600px] mx-auto space-y-8 pb-20">
              ...
              {!simulation.isCompliant && (
                <div className="rounded-xl border border-destructive bg-destructive/5 p-6 flex flex-col md:flex-row items-center gap-6 animate-in zoom-in-95 duration-300">
                    <div className="p-4 bg-destructive/10 rounded-full"><Lock className="h-8 w-8 text-destructive" /></div>
                    <div className="flex-1 text-center md:text-left">
                        <h3 className="text-lg font-bold text-destructive">{t('error_title')}</h3>
                        <p className="text-sm text-muted-foreground mb-2">
                                {simulation.complianceError === "MAX_LIMIT" 
                                    ? `${t('error_max_reached')} (${currency.format(simulation.maxBankFinancing)})`
                                    : simulation.complianceError}
                            </p>
                            <div className="inline-flex items-center gap-2 bg-background border px-3 py-1 rounded text-sm font-medium">
                            {t('error_missing')} <span className="text-destructive font-bold">{currency.format(simulation.shortfall || 0)}</span> {t('error_downpayment')}
                        </div>
                    </div>
                    <Button variant="destructive" onClick={() => setDownPayment(d => d + (simulation.shortfall || 0))}>
                        {t('btn_fix')}
